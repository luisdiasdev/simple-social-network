...
import type { PostCreateRequest, PostPictureResponse, PostResponse } from './types';
...
export const savePostPicture = (file: File) => {
  ...
  const promise = publicApi
    .post<PostPictureResponse>(PICTURE_ENDPOINT, formData, {
      headers: {
...
export const savePost = (request: PostCreateRequest) =>
  publicApi.post<PostResponse>(ENDPOINT, request).then((response) => response.data);
